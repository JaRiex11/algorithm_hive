        ...
        public:
            ...
            void init(int *argc, char *argv[]) {
                drawings_future = std::async(std::launch::async,[&, argc, argv]() {
                    ...
                    while(!is_shutdown) {
                        std::vector<std::shared_ptr<tools::Drawing>> snapshot;
                        {
                            std::lock_guard<std::recursive_mutex> lock(tools::Drawing::drawings_mutex);
                            tools::Drawing::purge_pending_closed_windows();
                            snapshot = tools::Drawing::drawings;
                        }
                        for (size_t i = 0; i < snapshot.size(); ++i) {
                            if (!snapshot[i]->pending_close)
                                snapshot[i]->update_window();
                        }
...
//------------------------------------------------------------------------------
        /** Ждать, пока пользователь закроёт все окна графиков (обработка в потоке GLUT). */
        static void wait_until_all_windows_closed() {
            using namespace tools;
            for (;;) {
                {
                    std::lock_guard<std::recursive_mutex> lock(tools::Drawing::drawings_mutex);
                    if (tools::Drawing::drawings.empty())
                        break;
                }
                std::this_thread::sleep_for(std::chrono::milliseconds(16));
            }
...
//------------------------------------------------------------------------------
        static int get_pos_plot(const std::string &name) {
            using namespace tools;
            std::lock_guard<std::recursive_mutex> lock(Drawing::drawings_mutex);
            for(size_t i = 0; i < Drawing::drawings.size(); ++i) {
                if(Drawing::drawings[i]->window_name == name) return (int)i;
            }
            return -1;
        }
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
        ...
        static void save_image(std::string window_name, std::string file_name) {
            ...
            if(pos >= 0 && !Drawing::drawings[pos]->pending_close) {
